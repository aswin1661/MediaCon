...
export const TextCombo = ({ header, sub, theme, className }: { header: string, sub: string, theme: "black" | "white", className?: string }) => {
    ...
    return (
        <div class={cn("text-start", className)}>
            <Text class="z-3 text-balance opacity-75 max-w-2xl" coloring={theme} size={"h2"} variant={"tedx"}>
            &nbsp{first}
                <span class="text-[#fed500]">{last}</span>
            </Text>
            <Text class="text-balance opacity-75 max-w-2xl" coloring={theme} size={"p"} variant={"tedx-sub"}>&nbsp &nbsp &nbsp{sub}</Text>
        </div>
    );
}